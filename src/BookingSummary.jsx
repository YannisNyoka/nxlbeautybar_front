...
const BookingSummary = ({
  ...
  if (!displayName || displayName.trim() === '' || displayName.includes('undefined') || displayName.trim() === 'undefined undefined') {
    if (user?.firstName && user?.lastName) displayName = `${user.firstName} ${user.lastName}`.trim();
    else if (user?.firstName) displayName = user.firstName.trim();
    else if (user?.lastName) displayName = user.lastName.trim();
  ...
  const handleConfirm = async () => {
    if (!localContact.trim()) { setError('Please enter your contact number before confirming.'); return; }
    if (!validatePhone(localContact)) { setError('Please enter a valid South African phone number (9–10 digits).'); return; }
    setError(''); setLoading(true); setApiError(''); setApiSuccess('');

    try {
      const token = localStorage.getItem('token');
      if (!token) { setApiError('You must be logged in to book an appointment.'); setLoading(false); return; }
      ...
      if (!employeeId) { setApiError('Invalid stylist selection. Please select a valid stylist.'); setLoading(false); return; }

      // Resolve serviceIds
      const serviceIds = (servicesList?.length
        ? selectedServices.map((sName) => { const svc = servicesList.find((s) => s.name === sName); return svc?._id || svc?.id || null; }).filter(Boolean)
        : []);
      if (!serviceIds.length) { setApiError('Invalid service selection. Please select at least one valid service.'); setLoading(false); return; }
      ...
      if (!appointmentResult.success) {
        ...
        if (errorMsg.toLowerCase().includes('not available') || errorMsg.toLowerCase().includes('already booked') || errorMsg.toLowerCase().includes('time slot')) {
      ...
      if (!appointmentId) { setApiError('Failed to retrieve appointment ID. Please try again.'); setLoading(false); return; }

      if (onBookingConfirmed) {
        onBookingConfirmed({
          appointmentId, _id: appointmentId,
          date: createdAppointment.date, time: createdAppointment.time,
          userName: displayName, serviceType: selectedServices.join(', '),
          duration: totalDuration, status: createdAppointment.status || 'booked',
        });
      }
      ...
      setTimeout(() => {
        navigate('/payment', {
          state: {
            name: displayName, dateTime, appointmentId,
      ...
      <div className="bs-modal">
        ...
        <div className="bs-section">
          ...
          <div className="bs-totals">
            ...
            <div className="bs-total-row bs-total-price"><span>Total Price</span><span>R{totalPrice}</span></div>
          </div>
        </div>
        <div className="bs-divider" />
        <div className="bs-section">
        ...
        <div className="bs-section">
          ...
          <div className="bs-client-row bs-phone-row">
            ...
            <div className="bs-phone-wrap">
              ...
              <input
                id="bs-phone" type="tel" value={localContact} onChange={handleContactChange}
          ...
          {apiSuccess && <div className="bs-success-msg">{apiSuccess}</div>}
        </div>
        <button className="bs-confirm-btn" onClick={handleConfirm} disabled={loading}>
          {loading ? <span className="bs-spinner-wrap"><span className="bs-spinner" /> Saving...</span> : 'Confirm & Proceed to Payment →'}
        </button>
        <p className="bs-note">A non-refundable booking fee of <strong>R100</strong> will be charged on the next step.</p>
      </div>
    </div>
  );
};
